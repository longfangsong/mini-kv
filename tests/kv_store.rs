use kvs::server::storage::lsm_tree::LSMTree as KvServer;
...
fn get_stored_value() -> Result<()> {
    ...
    let mut store = KvServer::new(temp_dir.path())?;
...
fn overwrite_value() -> Result<()> {
    ...
    store.insert("key1".to_owned(), "value1".to_owned())?;
    assert_eq!(store.get("key1")?, Some("value1".to_owned()));
    store.insert("key1".to_owned(), "value2".to_owned())?;
    ...
    let mut store = KvServer::new(temp_dir.path())?;
    assert_eq!(store.get("key1")?, Some("value2".to_owned()));
    store.insert("key1".to_owned(), "value3".to_owned())?;
