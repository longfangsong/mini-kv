...
fn run(opt: Opt) -> Result<()> {
    ...
    match engine {
        Engine::kvs => run_with_engine(LSMTree::new(current_dir()?)?, opt.addr),
        Engine::sled => run_with_engine(SledKvsStorage::new(sled::open(current_dir()?)?), opt.addr),
    }
}

fn run_with_engine<E: KvStorage + 'static>(engine: E, addr: SocketAddr) -> Result<()> {
